...
lang: en
page_kind: reference
title: "Track Spline Builder — Setup and Settings"
description: "Quick start and complete Track Physics Solver settings reference"
product: track
doc_section: guide
---


# Track Spline Builder: setup and complete settings

This page starts with a working setup and keeps the complete release-facing
`Track Physics Solver` settings reference below it. Blueprint-callable actions
are listed separately in the [Track Blueprint node reference]({{ '/docs/track-blueprint-nodes.en.html' | relative_url }}).

<a id="quick-start"></a>

## Quick start

For each vehicle side:

1. Add one Spline Component, one `Track Spline Builder`, and one `Track Physics Solver`.
2. In the Builder, assign `Source Spline`, `Track Link Mesh`, link count or spacing, and `Orientation Mode = Planar Track`.
3. In the Solver, assign the same `Target Spline`, the vehicle `Source Mesh Component`, wheel bone naming, counts, and radii.
4. Assign the front and rear end-wheel bones, then click `Generate Bone Rig Now`.
5. Assign the matching `Track Builder`, enable `Update Track Builder After Solve`, and click `Validate Setup`.
6. Run PIE and verify a closed loop, fixed link count, correct direction and speed, and no response to the vehicle body.

Keep `Surface Query Mode = World Static Only` for the normal visual-track
workflow. Use another collision query only when the project explicitly requires it.

<a id="solver-settings"></a>

## Complete component settings
