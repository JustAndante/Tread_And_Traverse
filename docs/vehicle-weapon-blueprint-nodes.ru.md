...
| `Get Resolved Smoke Launcher Setup` | Возвращает готовую runtime-геометрию smoke launcher, если она настроена в loadout. |
...
| Нода | Назначение |
|---|---|
| `Get Configured Weapon Muzzle Mesh And Socket (Advanced)` | Compatibility-адаптер для старых BP-функций, которым нужен именно Skeletal Mesh, а не общий Scene Component muzzle. |
