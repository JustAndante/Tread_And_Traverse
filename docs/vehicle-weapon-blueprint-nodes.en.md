...
description: "Current curated buyer-facing Blueprint API for Vehicle Weapon System"
lang: en
page_kind: reference
product: weapon
doc_section: nodes
...
| Node | Purpose |
|---|---|
| `Get Cached Vehicle Aim Data` | Returns the compact camera/AimCube frame already cached by the shared Weapon System update. |
...
| Node | Purpose |
|---|---|
| `Resolve Main Weapon Aim Channel` | Selects the standard aim source for the main weapon from the current gunner, commander, and double-fire camera modes. |
| `Resolve Auxiliary Weapon Aim Channels` | Selects yaw and pitch sources for an auxiliary or commander-mounted weapon without vehicle-specific branches. |
...
| Node | Purpose |
|---|---|
| `Get Configured Weapon Skeletal Muzzle (Advanced)` | Returns the configured muzzle as a Skeletal Mesh for integrations that explicitly require that component type. |
...
[Roadmap →]({{ '/docs/roadmap.en.html' | relative_url }})
</div>
